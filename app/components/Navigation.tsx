...
import {
  ...
  ShoppingBag, BarChart, Settings, LogOut, CalendarClock
} from 'lucide-react';
...
export default function Navigation() {
  ...
  useEffect(() => {
    if (user) {
      setEmployee({
        ...
        avatar: user.avatar || `https://api.dicebear.com/7.x/avataaars/svg?seed=${user.name || user.email || 'User'}`
      });
    }
  }, [user]);
  ...
  const menuItems = [
    ...
    { id: 'employees', path: '/employees', label: 'Employee', icon: User, roles: ['ADMIN', 'MANAGER'] },
    { id: 'workshift', path: '/employees/workshift', label: 'Workshift', icon: CalendarClock, roles: ['ADMIN', 'MANAGER'] },
