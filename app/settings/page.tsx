...
export default function SettingsPage() {
  ...
  const triggerSync = async () => {
    try {
      await systemService.syncSeats('all');
      toast.success("Đã kích hoạt đồng bộ ghế thời gian thực!");
    } catch (error) {
      toast.error("Lỗi khi đồng bộ ghế.");
    }
  };
    ...
    <RoleGuard
      ...
      fallback={
        <div className="flex h-[80vh] flex-col items-center justify-center gap-4">
          ...
          <p className="text-gray-500 font-medium">Bạn không có quyền truy cập vào khu vực cấu hình hệ thống.</p>
        </div>
      }
    >
      <div className="w-full max-w-[1600px] mx-auto min-h-screen pb-20">
        ...
        <div className="fixed bottom-8 right-8 flex gap-3 animate-in slide-in-from-bottom-4 duration-500">
          ...
          <button onClick={handleUpdateSettings} className="bg-indigo-600 text-white px-8 py-3 rounded-2xl font-black text-sm flex items-center gap-2 shadow-xl shadow-indigo-200 hover:bg-indigo-700 transition-all">
